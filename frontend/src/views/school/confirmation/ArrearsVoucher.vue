...
async function load(print = false) {
  voucherNo.value = voucherNo.value.trim().toUpperCase()
  loading.value = true
  errorMessage.value = ''
  try {
    ...
    voucher.value = null
    errorMessage.value = getErrorMessage(error, '查询欠费确认单失败，请稍后重试。')
  } finally {
    loading.value = false
  }
}
...
<template>
  <section class="voucher-page">
    ...
    <section class="search-panel">
      <label><span>单据编号</span><input v-model.trim="voucherNo" placeholder="例如 GC2026000354" @keyup.enter="load()" /></label>
    ...
    <p class="voucher-format">格式：GC + 4位确认年份 + 至少6位申请ID，例如申请ID为354时生成 GC2026000354。</p>
...
.voucher-format { margin: 8px 0 0; color: #909399; font-size: 12px; }
</style>
