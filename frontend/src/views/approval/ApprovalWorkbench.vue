...
const metrics = computed(() => [
  ...
  { label: '退回修改', value: dashboard.value.returned, hint: '学生修改后可重新流转', icon: Warning, tone: 'danger' },
...
async function submitReview(payload) {
  ...
  try {
    ...
    ElMessage.success(`审核意见已提交，当前状态：${result.statusName}`)
...
async function resubmitReturned(row) {
  try {
    ...
    ElMessage.success(`补交成功，当前状态：${result.statusName}`)
    await loadWorkspace()
  } catch (error) {
    if (error !== 'cancel') ElMessage.error(error.message || '补交失败')
  } finally {
  ...
  <div class="page-container workbench-page">
    <section class="page-heading-row">
      <div>
        ...
        <p>{{ role === 'COUNSELOR' ? '核验学生材料、确认补助金额，并按批次统一提交学院。' : '复核本学院申请、检查名额额度，并在截止时间前提交学校。' }}</p>
      </div>
    ...
    <section v-if="submission" class="submission-bar" :class="{ completed: submission.initialSubmitted }">
