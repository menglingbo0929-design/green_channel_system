...
watch(() => props.modelValue, (open) => {
  ...
  } : { resource: props.resourceKind || 'GIFT', scope: 'COLLEGE', targetId: null, total: null, version: null })
})
...
<template>
  <el-dialog :model-value="modelValue" :title="editing ? '编辑名额 / 额度分配' : '新增名额 / 额度分配'" width="560px" class="business-dialog" @update:model-value="emit('update:modelValue', $event)">
    ...
    <el-form class="allocation-form" label-width="100px">
      <el-form-item label="资源类型"><el-radio-group v-model="form.resource" :disabled="!!editing || !!resourceKind"><el-radio-button value="GIFT">礼包名额</el-radio-button><el-radio-button value="SUBSIDY">补助额度</el-radio-button></el-radio-group></el-form-item>
