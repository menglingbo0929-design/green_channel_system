...
public interface ApplicationResourceMapper {
    ...
    @Select("SELECT COUNT(*) FROM batch_gift_item WHERE id=#{id} AND batch_id=#{batchId} AND deleted=0 " +
            "AND per_student_limit>=#{quantity} AND stock_total-reserved_count-used_count>=#{quantity}")
    ...
    @Update("UPDATE batch_gift_item SET reserved_count=reserved_count+#{quantity} WHERE id=#{batchGiftItemId} AND deleted=0 AND stock_total-reserved_count-used_count>=#{quantity}")
    int reserveGiftStock(@Param("batchGiftItemId") Long batchGiftItemId, @Param("quantity") int quantity);
    @Update("UPDATE college_gift_quota SET reserved_count=reserved_count+#{quantity} WHERE batch_id=#{batchId} AND college_id=#{collegeId} AND deleted=0 AND quota_total-reserved_count-used_count>=#{quantity}")
    int reserveCollegeGiftQuota(@Param("batchId") Long batchId, @Param("collegeId") Long collegeId, @Param("quantity") int quantity);
    @Update("UPDATE grade_gift_quota SET reserved_count=reserved_count+#{quantity} WHERE batch_id=#{batchId} AND grade_id=#{gradeId} AND deleted=0 AND quota_total-reserved_count-used_count>=#{quantity}")
    ...
    @Update("UPDATE college_subsidy_quota SET reserved_amount=reserved_amount+#{amount} WHERE batch_id=#{batchId} AND college_id=#{collegeId} AND deleted=0 AND quota_amount-reserved_amount-used_amount>=#{amount}")
    int reserveCollegeSubsidyQuota(@Param("batchId") Long batchId, @Param("collegeId") Long collegeId, @Param("amount") BigDecimal amount);
    @Update("UPDATE grade_subsidy_quota SET reserved_amount=reserved_amount+#{amount} WHERE batch_id=#{batchId} AND grade_id=#{gradeId} AND deleted=0 AND quota_amount-reserved_amount-used_amount>=#{amount}")
