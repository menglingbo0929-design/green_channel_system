...
- 状态：PROPOSED
- 提出人：成员二
- 负责人：成员二（申请、资源和附件接口）、成员一(APROVED)（身份、学生、批次和基础数据接口）、成员三（状态流转和审核记录接口）、成员四（确认、代申请、补录和统计接口）
- 影响模块：全部模块
- 影响表：`application`、申请详情表、礼包/库存/名额/补助额度表、`approval_record`、`arrears_confirmation` 及基础数据表
- 影响接口：学生申请、申请配置、跨模块申请状态写入、资源生命周期、欠费确认读取、代申请、补录和统计查询
- 影响状态：使用现有申请状态；需要确认首次提交、退回重提、确认完成和非欠费业务办结边界
- 变更内容：新增 `docs/decisions/application-config.md`，整理成员二依赖成员一/三的能力、成员二向成员三/四提供的 Service、成员二 REST 接口、错误码及开发前阻塞项。
- 使用者需要执行的操作：成员一确认身份、学生、批次和组织查询接口；成员三确认提交/重提及申请状态写入契约；成员四确认待确认申请、代申请、补录和统计契约；四人确认批次关联、补助预占、幂等和办结规则。
- 对应提交：待提交
