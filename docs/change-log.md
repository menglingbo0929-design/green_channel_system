...
- 身份：学校代申请、线下补录、欠费确认、单据、统计和报表统一从成员一 JWT 登录上下文取得当前用户，前端不再发送 `X-User-Id`。
- 待确认列表：成员四 `ApplicationReadPortAdapter` 按当前页学生 ID 一次批量调用成员一 `StudentOrganizationSnapshotQuery`，补齐学号、姓名、学院、专业、年级和班级；不新增学生 Mapper，不逐条查询。
- 统计页面：删除个人列方案、重复批次框及非正式指标卡；复用成员一组织接口补齐学院/专业/年级/班级筛选，并展示欠费原因人数与金额、礼包物品申请数量。
- 空状态：待确认列表明确表示当前数据库没有进入 `CONFIRM_PENDING` 的申请，不以模拟数据代替前序状态。
- 验证：JDK 21 下后端 `compile` 成功，前端生产构建成功；未启动或调试用户服务。

## 2026-07-22｜成员三审批表基线与取消回归测试收尾

- 状态：`IMPLEMENTED`（成员三独立范围）。
- 数据库调整：主建库脚本中的 `approval_submission_record` 已与成员三迁移和 Mapper 统一，采用 `batch_type + green_channel_batch_id + subsidy_batch_id`，并补齐批次引用、审核轮次和提交数量约束。
- 测试数据调整：保留原有 100 条批量上报样本，通过临时种子表转换为新字段结构；为 100 条消息已读记录补齐对应的 `system_message`，保证接收人、消息 ID 和已读数据一致，并增加退回、拒绝、通过、线下办理四条未读样本。
- 回归测试：新增学校取消的申请版本冲突测试，以及资源释放失败时不继续作废单据、写审核记录或发送消息的测试。
- 验证：成员三 SQL 契约静态校验和前端生产构建通过；后端测试仍在主代码编译阶段受共享缺失接口 `ISupplementApplicationService` 阻塞。

## 2026-07-22｜成员三学校取消页面接线

- 状态：`IMPLEMENTED`（成员三前端范围）。
- 变更内容：在学校审核工作台“已审核”列表和申请详情中，为 `APPROVED/CONFIRM_PENDING/COMPLETED` 申请增加“取消申请”入口；复用业务确认弹窗收集取消原因，调用真实取消 API，并携带当前版本与唯一请求 ID。
- 一致性处理：取消成功后关闭弹窗和详情并刷新工作台；失败时展示后端错误且不提前修改页面状态。模拟接口同步增加版本冲突、请求幂等和取消审核记录展示。
- 边界：前端不直接释放资源或作废单据，真实执行仍由成员三后端事务协调成员二资源能力和成员四单据能力。
