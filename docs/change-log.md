...
- 状态：PROPOSED
- 提出人：成员二
- 负责人：成员二（申请、资源和附件接口）、成员一（已确认身份、学生、批次和基础数据接口）、成员三（已完成附条件评审，等待复核）、成员四（确认、代申请、补录和统计接口）
- 影响模块：全部模块
...
- 使用者需要执行的操作：成员三复核修订后的接口；成员四确认待确认申请、代申请、补录、统计及外层事务契约；成员一和成员四确认两类批次外键方案；确认后更新为 `APPROVED`。
- 对应提交：待提交
