...
学校取消的后端接口、真实 API 方法和页面接线均已完成：学校审核工作台“已审核”列表及申请详情会在 `APPROVED/CONFIRM_PENDING/COMPLETED` 状态展示取消入口，复用业务确认弹窗填写原因，并提交 `reason + version + requestId`。成功后刷新工作台与详情；失败时保留弹窗并展示后端错误。前端不自行释放资源、作废单据或先行显示取消成功，始终以后端事务结果为准。
