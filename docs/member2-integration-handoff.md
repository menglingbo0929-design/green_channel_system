...
## 2026-07-22｜本工作区追加完成项

- 学生端已具备私有目录附件上传及正式提交：仅允许申请本人对 `STUDENT` 来源的草稿或退回申请操作；附件限制为 PDF/JPG/JPEG/PNG、最大 10 MiB，并同时校验扩展名、MIME 和文件头。提交在同一事务内按“附件校验 → 资源预占 → `ApprovalTransitionService.submitInitial`”执行。
- 绿色通道资格判断复用成员一的 `StudentProfileQueryService` 与 `BatchQueryService`，覆盖无开放批次、申请时间、年级、资料完整度、贷款条件和重复申请六项规则；学生新建草稿时由后端同步校验，不能绕过页面。已有申请时，学生中心直接提示继续编辑或正式提交。
- 学生申请中心已展示资格结果、禁用不符合条件的新建入口，并自动带入当前开放批次；确认提交前选择证明附件，提交后进入辅导员审核。
- 仍使用 `X-Student-Id`、`X-User-Id` 作为本地开发身份占位。成员一提供可信会话/JWT 后，必须替换这些客户端可伪造的请求头；这不影响当前成员二业务编排的完成度。
